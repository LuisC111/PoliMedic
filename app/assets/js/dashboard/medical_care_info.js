...
function listarMedico(){
    var parametros = {
        casoConsulta: 'listPhysician_type',
        };
    $.ajax({
        url: "../app/inc_php/dashboard/datosDashboard.php",
        type: "POST",
        ...
        data: parametros,
        success: function(response) {
                var select = $('#selPhysician_type');
                select.find('option').remove();
                $.each(response.combos, function(i, item) {
                    select.append('<option value="' + item.id + '">' + item.physician_type + '</option>');
                });
                var select2 = $('#selPhysician_type-member');
                select2.find('option').remove();
                $.each(response.combos, function(i, item) {
                    select2.append('<option value="' + item.id + '">' + item.physician_type + '</option>');
                });
        },
        error: function(jqXHR, textStatus, errorThrown) {
            //alert("Error en solicitud Ajax de | "+jqXHR.responseText + " | " + textStatus + " | " + errorThrown);
...
function addMedicalCareInfo(){

    var parametros = {
        casoConsulta: 'addMedicalCareInfo',
        valorConsulta: $('#hidIdMember').val(),
        familycore_id: $('#hidIdFamily').val(),
        physician_type : $("select[name=selPhysician_type]").val(),
        medical_appointment: $('#medical_appointment').val(),
    ...
    $.ajax({
        ...
        success: function(response) {
            swal.fire({
                title: '<center>Control de Salud registrado</center>',
                icon: 'success',
                ...
                $('#modal-agregar').modal('hide');
                $('#modal-agregar-member').modal('hide');
                $("input[name=medical_appointment]").val('')
                $("input[name=physician_name]").val('')
                $("input[name=observation]").val('')
                $("input[name=medical_appointment-member]").val('')
                $("input[name=physician_name-member]").val('')
                $("input[name=observation-member]").val('')
                listarSolicitudes($('#divTblSolicitudes'), $('#tblSolicitudes'));
                listarSolicitudesMember($('#divTblSolicitudesMember'), $('#tblSolicitudesMember'));
                listarSolicitudesOwner($('#divTblSolicitudesOwner'), $('#tblSolicitudesOwner'));     
            }); 
...
function addMedicalCareInfoMember(){

    var parametros = {
        casoConsulta: 'addMedicalCareInfoMember',
        valorConsulta: $('#hidIdMember').val(),
        familycore_id: $('#hidIdFamily').val(),
        idMember: $("select[name=selMember]").val(),
        physician_type : $("select[name=selPhysician_type-member]").val(),
        medical_appointment: $('#medical_appointment-member').val(),
        physician_name: $('#physician_name-member').val(),
        observation: $('#observation-member').val()
    };
    $.ajax({
        url: "../app/inc_php/dashboard/datosDashboard.php",
        type: "POST",
...
function detallarSolicitud(solicitud) {
    ...
    $('#divAlertModal').hide();
    var parametros = {
        casoConsulta: 'tblMedicalCare_info_admin_detail',
...
function listarSolicitudes(divControl, control) {
    ...
    var parametros = {
        casoConsulta: "tblMedicalCare_info_admin"
            };
    $.ajax({
        url: "../app/inc_php/dashboard/datosDashboard.php",
        type: "POST",
...
function listarSolicitudesOwner(divControl, control) {
    ...
    var parametros = {
        casoConsulta: "tblMedicalCare_info_owner",
        valorConsulta: $('#hidIdFamily').val()
            };
    $.ajax({
        url: "../app/inc_php/dashboard/datosDashboard.php",
