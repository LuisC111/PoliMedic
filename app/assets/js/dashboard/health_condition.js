...
function addHealthCondition(){
    let file = $("#lab_file_path").prop("files")[0];
    let extension = $("#lab_file_path").val().split('.').pop().toLowerCase();

    var parametros = {
        casoConsulta: 'addHealthCondition',
        valorConsulta: $('#hidIdMember').val(),
        lab_type: $("select[name=selLabType]").val(),
        extension: extension,
        family_core: $("#hidIdFamily").val(),
        common_disease: $("select[name=selCommonDisease]").val(),
        particular_desease: $("input[name=particular_desease]").val()
    ...
    $.ajax({
        ...
        success: function(response) {
            uploadFile(response.combos, file);
        },
        error: function(jqXHR, textStatus, errorThrown) {
            //alert("Error en solicitud Ajax de | "+jqXHR.responseText + " | " + textStatus + " | " + errorThrown);
            var msgError = "Error en solicitud Ajax de consulta | " + jqXHR.responseText + " | " + textStatus + " | " + errorThrown;
...
function uploadFile(solicitud, file){
    var data = new FormData();
    data.append('archivo', file);
    data.append('solicitud', solicitud);

    $.ajax({
        url: "../app/inc_php/dashboard/uploadFile.php",
        type: "POST",
        contentType: false,
        data: data,
        processData: false,
        ...
        if (data.ok) {
            swal.fire({
                title: '<center>Condiciones de Salud Registradas</center>',
                icon: 'success',
                ...
                $("input[name=particular_desease]").val('')
                $("input[name=lab_file_path]").val('')
                $("input[name=particular_desease-member]").val('')
                $("input[name=lab_file_path-member]").val('')
                listarSolicitudes($('#divTblSolicitudes'), $('#tblSolicitudes'));
                listarSolicitudesMember($('#divTblSolicitudesMember'), $('#tblSolicitudesMember'));
                listarSolicitudesOwner($('#divTblSolicitudesOwner'), $('#tblSolicitudesOwner'));
...
function addHealthConditionMember(){
    let file = $("#lab_file_path-member").prop("files")[0];
    let extension = $("#lab_file_path-member").val().split('.').pop().toLowerCase();

    var parametros = {
        casoConsulta: 'addHealthConditionMember',
        valorConsulta: $('#hidIdMember').val(),
        idMember: $("select[name=selMember]").val(),
        lab_type: $("select[name=selLabType-member]").val(),
        extension: extension,
        family_core: $("#hidIdFamily").val(),
        common_disease: $("select[name=selCommonDisease-member]").val(),
        particular_desease: $("input[name=particular_desease-member]").val()
...
function detallarSolicitud(solicitud) {
    ...
    $('#divAlertModal').hide();
    var parametros = {
        casoConsulta: 'tblHealth_condition_admin_detail',
...
function listarSolicitudes(divControl, control) {
    ...
    var parametros = {
        casoConsulta: "tblHealth_condition_admin"
            };
    $.ajax({
        url: "../app/inc_php/dashboard/datosDashboard.php",
        type: "POST",
...
function listarSolicitudesMember(divControl, control) {
    ...
    var parametros = {
        casoConsulta: "tblHealth_condition_member",
        valorConsulta: $('#hidIdMember').val()
            };
    $.ajax({
        url: "../app/inc_php/dashboard/datosDashboard.php",
...
function listarSolicitudesOwner(divControl, control) {
    ...
    var parametros = {
        casoConsulta: "tblHealth_condition_owner",
        valorConsulta: $('#hidIdFamily').val()
            };
    $.ajax({
        url: "../app/inc_php/dashboard/datosDashboard.php",
